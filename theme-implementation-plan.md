...
**Dernière mise à jour :** 2026-08-14 18:34 (Europe/Paris)
...
- 2026-08-14 — Étape 5 validée : les sept patterns `hero`, `text-image`, `cards`, `network-farm-selector`, `columns`, `text-image-motif` et `graphic-band` sont disponibles automatiquement dans la catégorie `Sections Le Paysan Urbain`. Les structures utilisent les blocs cœur, les tokens de `theme.json`, les classes `lpu-band`/`lpu-card-grid`/`lpu-motif-*` et des règles responsives partagées ; `cards` est réutilisable pour les deux grilles de la Home. Le visuel SVG local `pattern-placeholder.svg` sert uniquement de média de démonstration remplaçable dans Gutenberg : aucune photo de maquette ni contenu n’a été injecté dans `Accueil` ou dans le bootstrap. Le registre WordPress, l’endpoint de l’inséreur en contexte administrateur et le rendu serveur des sept patterns ne signalent aucun bloc invalide. Une page de contrôle temporaire avec huit insertions dans l’ordre de `sections-Home.txt` a été inspectée avec `web-inspector` à `1440px` et `390px` ; les réponses sont HTTP 200, sans erreur console, erreur de page, requête échouée ou défaut responsive bloquant. La page de contrôle a ensuite été placée à la corbeille et les quatre pages `Accueil` ont été vérifiées inchangées.
