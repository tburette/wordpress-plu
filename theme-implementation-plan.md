...
- Les réglages spécifiques au site dans le thème sont indexés par une clé dérivée du sous-domaine, et non par le `blog_id` attribué par l'installation. L’ajout d’un site ne doit pas nécessiter de nouvelle branche conditionnelle dans le thème ; les variantes de logo suivent la convention documentée dans `functions.php`.
...
- il y a un tout petit espace entre certaines sections (à analyser manuellement par Thomas)
 
...
- Shopify Paris et tout e-commerce intégré. Un simple lien externe vers la boutique n'est pas assez.
...
- Les tailles `contentSize` et `wideSize` doivent être contrôlées et peutêtre ajustées. Il faut comparer avec la maquette.
...
Appeler `scripts/content/setup.sh` depuis `setup-multisite-network.sh` après la création des sites. Le bootstrap multisite est déclenché explicitement par `npm run env:multisite:setup`, et non automatiquement au démarrage de `wp-env`. Chaque script de contenu conserve ses données Gutenberg dans son propre répertoire, hors du code shell. Un redémarrage ne doit jamais réinitialiser une page ou un menu déjà édité dans Gutenberg.
